...
xlib::logger::LogWriter::~LogWriter() = default;
...
void xlib::logger::LogWriter::if_found_config_file_failed() {
    ...
    sink_manager_.init_manager(sink_manager_init);
    sink_manager_.registration(std::make_tuple(xlib::Flags::ShowSinkFlag::Show_None, SinkDataStructure()));
    std::cerr << "Configure by file failed, logger will be silently..." << std::endl;
}

void xlib::logger::LogWriter::log() const {
